...
/*********
//
// FunctionScanPort() - Lấy địa chỉ IP và Port để kiểm tra xem nó có mở không.
//
*********/
static BOOL FunctionScanPort(char* ip, int port)
{
	SOCKET sock;
	...
	{
		cout << "Socket return error, code: " << WSAGetLastError() << endl;
		closesocket(sock);
		return FALSE;
	}
...
/*********
//
// task() - In ra kết quả scan đối với thread
//
********/
VOID task(char* ip, unsigned int port) {
	if (FunctionScanPort(ip, port))
	{
		cout << port << " : OPEN : ";
		ShowPortNumberInfo(port);
		cout << endl;
...
/*********
//
// ScanRangePortThread() - Tạo thread chính để scan.
//
*********/
VOID ScanRangePortThread(char* ip, int portInit, int portEnd) {
	vector<thread*> tasks;
	for (int p = portInit; p <= portEnd; p++) {
		tasks.push_back(new thread(FunctionScanPort, ip, p));
	}
...
/*cout << ip << ":" << portInit << "-" << portEnd << endl;*/

/*********
//
// GetIPFromTarget() - chuyển đổi domain thành dãy ip(chưa hoàn thiện)
//
*********/
static BOOL ConvertIPFromTarget(char* address) {
	hostent* host = gethostbyname(address);

	if (host != NULL)
	{
		in_addr in;
		memcpy_s(&in, sizeof(in_addr), host->h_addr_list[0], sizeof(in_addr));
		strcat_s(ip, MAX_PATH, inet_ntoa(in));
		return TRUE;
	}
	cout << "Can't conver IP from target: " << host << "failed with error: " << WSAGetLastError() << endl;
	return FALSE;
}
...
int main()
{
	...
	// check option
	cout << "--------------Menu--------------" << endl;
	cout << "1. Scan specifical TCP port" << endl;
	cout << "2. Scan range TCP port" << endl;
	//cout << "3. Scan all TCP port" << endl;
	cout << "option: " << flush;
	cin >> option;
	...
	if (option == 1) {
		cout << "Port: " << flush;
		cin >> port;
		cout << "Showing open ports on " << address << "...\n";
		cout << "PORT : STATUS : SERVICE" << endl;
		if (port != NULL) {
			FunctionScanPort(address, port);
		}
	}
	else if (option == 2) {
		cout << "Start Port: " << flush;
		cin >> StartPort;
		cout << "End Port: " << flush;
		cin >> EndPort;
		cout << "Showing open ports on " << address << "...\n";
		cout << "PORT : STATUS : SERVICE" << endl;
		if (EndPort != NULL) {
			cout << "Start scan range port: " << StartPort << " to " << EndPort << endl;
			ScanRangePortThread(address, StartPort, EndPort);
		}
	}
	else {
		cout << "Showing open ports on " << address << "...\n";
		cout << "PORT : STATUS : SERVICE" << endl;
		ScanRangePortThread(address, 0, 65535);
	}
